...
namespace parc {
  ...
  class ProgramStepEmpty : public ProgramStep<TLOGGER> {
  public:
    ProgramStepEmpty(TLOGGER& logger, uint8_t key) : ProgramStep<TLOGGER>(logger, 1), _key(key) {}
  protected:
    void onTick() {
      //ProgramStep<TLOGGER>::_log.print("ProgramEmpty "); ProgramStep<TLOGGER>::_log.println(_key);
      //ProgramStep<TSERIAL>::_log.print(" free mem:"); ProgramStep<TSERIAL>::_log.println(freeMemory());
    }
  private:
    uint8_t _key;
  };
  ...
  public:
    ...
    void dispose() {
      if (ProgramStep<TLOGGER>::_next != 0) {
        ProgramStep<TLOGGER>::_next->dispose();
