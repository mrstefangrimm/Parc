...
protected:
  ...
  struct ReadingProgramCode : public tsmlib::BasicState<ReadingProgramCode, StatePolicy, true>, public tsmlib::SingletonCreator<ReadingProgramCode> {
    ...
    void entry(const Trigger::Timeout& ev, Identity<Trigger::Timeout>) {
      _subState = GotNothing;
      auto ao = static_cast<TerminalAo*>(ev.owner);
      ao->clearReadBuffer();
      _subState = static_cast<SubState>(ao->onStateReadingProgramCodeEntry());
    }
  };
  ...
  struct ReadingPin : public tsmlib::BasicState<ReadingPin, StatePolicy, true>, public tsmlib::SingletonCreator<ReadingPin> {
    ...
    void entry(const Trigger::Timeout& ev, Identity<Trigger::Timeout>) {
      auto ao = static_cast<TerminalAo*>(ev.owner);
      ao->clearReadBuffer();
      readingDone = ao->onStateReadingPinEntry();
    }
  };
  ...
  {
    bool eval(const ReadingProgramCode& state, const Trigger::Timeout& ev) {
      return state._subState == ReadingProgramCode::GotProgram;
    }
  };
  ...
  {
    bool eval(const ReadingProgramCode& state, const Trigger::Timeout& ev) {
      return state._subState == ReadingProgramCode::GotPin;
    }
  };
  ...
  uint8_t onStateReadingProgramCodeEntry() {
    auto log = TLOGGERFAC::create();
    ...
    while (_serial.available()) {
      ...
      if (ch == ':') {
        ...
        if (_keyPadInput.isPin) {
          ...
          return ReadingProgramCode::GotPin;
        }
        ...
        if (0 <= idx && idx < NumberOfPrograms) {
          ...
          prog.dispose();
          return ReadingProgramCode::GotProgram;
        }
        _serial.println(F(" This ain't dull, bye."));
        return ReadingProgramCode::GotNothing;
      }
